...
Arithmetic with [`calc()`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/calc) for the scale, `if()` for the rest. The variable is read right where it is needed, with no selector for the state at all. In my tests, its cost was roughly similar to a plain class selector. Rust never sees a heart. It doesn't know what `liked` looks like, and it shouldn't.

There is a browser-support limit: `if()` is Chromium-only today. In other current browsers the demo falls back to a style query on the same variable, which was a bit slower in my tests:
...
This is a direct write to CSS, completely bypassing the document. In my measurements, it was roughly as cheap as toggling a CSS class. The generated stylesheet also registers the variable, giving it a strict type and a default:
...
Crossing the border between those two worlds sounds expensive. In my measurements, however, small numeric calls were cheap - it is converting strings that takes real work.
...
- **The dark arts of Wasm optimization.** I enabled Link-Time Optimization (`lto = true`), restricted compilation to a single codegen unit, set `panic = "abort"`, and ran `wasm-opt -Oz` as a post-build step. Hilariously, setting Rust's `opt-level = "z"` produced a smaller binary but a *larger* Brotli-compressed file, so I reverted to the default `opt-level`.
...
So I measured it in Chrome. On a list of ten thousand rows, appending one more got about four times slower than with the old counter. So much for my assumption that each row could handle this independently. The newest CSS function in this post came last in this test.
...
A small Wasm lesson. In an earlier build, checkpoint cleanup pulled extra tree-removal code into the Wasm module through a single call to `BTreeMap::remove`. To shrink the module, I had to stop calling that method. Instead of removing old checkpoints, we now rebuild the tree from the ones we keep. Same retained checkpoints, less machinery to ship.
</Details>
...
This is exactly why the time travel slider flawlessly works on Brunhilda too. Replaying the commands, sensor reports, and ticks in order gives us the exact same robot state. Even the simulation speed is just state: "faster" and "slower" are ordinary inputs, identical to a click on the like button.
...
And that concludes our third app. Behold, the ROBOT! Or, to be precise, the 144 KB robot runtime (46 KB Brotlied), including its imported manifest. She cleans. She docks. She stops when told. With the naive pathfinding policy, she still aggressively attacks me. That is the whole feature list.
