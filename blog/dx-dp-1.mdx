...
What the non-delegatable share (`s`) measures is what is left over: the glance at `/showcase` to see if the thing rendered, the backstop check when the type checker cannot carry the case, reading prompt outputs by eye, reviewing the change summary when an agent flags it, approving security prompts, and writing the code myself on the few cases where prompting it would cost more than typing it.

Part 0 put `s` at 0.5. This list - short, deliberate, mostly visual - drove it to roughly 0.01.

There is a trade in those numbers worth recapping. The formula is not a constraint to minimise; it is a system to rebalance.

The most useful move - already named at `/showcase` and used throughout - is to push verification *off your side and onto the agent's*: typecheck and lint inside the agent's loop, the browser MCP reading its own console, the OpenAPI generator catching contract drift at compile time. Each one makes the agent take a little longer per task and your verification correspondingly shorter. If the agent slows down enough that its throughput slips behind yours, the answer is one more agent - and that costs almost nothing here, because context switching is half a minute.

A comprehensive test suite the agent actually runs is the strongest version of this trade. The eval harness from earlier is the version I still owe myself.

When verification sits on the agent's side of the line, *slower agents are not a problem - they are room to push your verification time further down and your agent count further up.* The trade only pays when context switching is cheap; where it is expensive (team coordination, big stateful contexts to reload), slower agents are just slower agents.

These are normal architectural choices. They *also* shrink the agent's job enough that I am the one running ahead, not waiting. The lever framing is a way to notice when a design choice is doing double duty.

Part 0 hinted at the flip side, and this recipe has tried to honour it: pulling levers harder than necessary stops paying. Tighter verification *on your side* raises your verification time until you are no longer catching anything new (the agent-side trade above has a different ceiling, set by how much real signal your tests and checks can carry). Longer prompts raise your prompt-writing time until you are doing the agent's job by hand. More agents raise your context-switching cost, because parallel work eats your attention faster than you would expect. The point is not to drive your time per task to zero - it is to drive it below what the agent side takes, and stop.

And the recipe itself is not free. The instruction file accumulates - by the end of this post it carries close to a dozen rules, and a few have started to caveat each other (regenerate the OpenAPI types, except the SSE handlers; build the component and stylesheet together, except when you do not). Plain CSS means no component library on the day you want a complicated table or a date picker. Two repos means no atomic commit when an endpoint changes shape. The Codex slot of the workflow ties one pane to a vendor. The brake from earlier is a permanent tax on verification time. Each one pays for itself elsewhere in lever savings, but they are real bills, and a recipe that pretends otherwise will mislead anyone copying it.

Then, sometimes, work on yourself instead of the workflow 😉.

## Running multiple agents

Part 0's parallelism rule said adding agents stops helping past a certain crossover, because each extra agent grows your context-switching cost. That assumed two things: every agent needs periodic attention from you, and the switching cost is the same regardless of what each agent works on.

With this recipe both soften. Most agents do not need periodic attention. And the switching cost shrinks when the tasks resemble each other - switching between two components is almost free; switching between a web-to-PDF microservice and a bookmark-icon background is the real cost Part 0 was modelling. The work splits naturally into two task types that parallelise cleanly, each mapped onto its own tool.
...
The two types do not compete for attention; they sit on different stretches of the pipeline. Client-side work tends to run on **two features at the same time**: Feature A is in component-building mode, several agents building presentational pieces in parallel against `/showcase`; Feature B (whose components landed yesterday) is in wiring mode, another agent or two hooking those existing components up to the backend. You spend your context-switching budget on the few decisions that actually need a human in the loop, and let everything else run.
...
In my paper-reader sessions, an agent takes roughly fifteen minutes end-to-end on a feature - prompt, generation, the iteration loop from the previous section, the lot - so agent time per task is fifteen minutes, the same convention Part 0 used. Prompting takes about a minute; verifying on `/showcase` plus a glance at the diff is around two; context switching between terminals lands around half a minute. The non-delegatable share, which the rest of this post has been chipping away at, ends up around 0.01.

In formula terms: `x = 15`, `a = 1`, `v = 2`, `c ≈ 0.5`, `s = 0.01`.
